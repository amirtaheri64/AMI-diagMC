...
print
#End
file = open('out_3rd_order.dat', 'w')
for i_files in range(6,7):
  ...
  ###################################
  #Begin
  file_name_ami_in='ami_in_m_'+str(3)+'_num_'+str(i_files)
  print 'And for this diagram:'
  print file_name_ami_in
  with open(file_name_ami_in,"rb") as fp:
    ami=pickle.load(fp)
  #print 'ami_in = ', ami
  file_name_F='f_m_'+str(3)+'_num_'+str(i_files)
  ...
  txt=str(3) + '\t' + str(i_files) + '\t' + str(N) + '\t' + str(T) + '\t' + str(re_fin) + '\t' + str(er_re) + '\t' + str(im_fin) + '\t' + str(er_im) + '\n'
  file.write(txt)
file.close()
