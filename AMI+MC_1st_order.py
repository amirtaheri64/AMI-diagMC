...
def Integrand(arrs,M,F):
  ...
  if len(r_freq[0])!=0:
    ...
    #print '?????'   
    #print 'G_val = ', G_val
  out.append((-1)**(M+F)*U*G_val.real/4**M/pi**(2*M))
  out.append((-1)**(M+F)*U*G_val.imag/4**M/pi**(2*M))
  #print out
  return out
...
###################################
#Begin
file_name_ami_in='ami_in_m_'+str(1)+'_num_'+str(1)
with open(file_name_ami_in,"rb") as fp:
  ami=pickle.load(fp)
print 'ami_in = ', ami
file_name_F='f_m_'+str(1)+'_num_'+str(1)
